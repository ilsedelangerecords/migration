...
class GitHubMigrator:
    ...
    def extract_content_from_html(self, html_content):
        ...
        if keywords_match:
            content['metadata']['keywords'] = keywords_match.group(1).strip()
        
        return content
      def scan_directory_recursively(self, dir_path=""):
        """Recursively scan a directory for all files"""
        try:
            url = f"{self.repo_url}/contents/{dir_path}" if dir_path else f"{self.repo_url}/contents"
            response = requests.get(url, headers=self.headers)
            
            if response.status_code == 200:
                return response.json()
            else:
                logger.warning(f"⚠️ Could not access directory {dir_path}: {response.status_code}")
                return []
        except Exception as e:
    ...
    def scan_for_lyrics_data(self, structure):
        """Scan repository for lyrics content"""
        logger.info("Scanning for lyrics data...")
        ...
        for item in structure:
            if item['type'] == 'file':
                ...
                # Look for files that might contain lyrics
                if any(keyword in name for keyword in ['lyric', 'song', 'text']):
                    logger.info(f"Found potential lyrics file: {item['name']}")
                    content = self.fetch_file_content(item['path'])
                    
                    if content:
                        if name.endswith('.json'):
                            try:
                                data = json.loads(content)
                                lyrics_data.extend(data if isinstance(data, list) else [data])
                            except json.JSONDecodeError:
                                logger.warning(f"⚠️ Invalid JSON in {item['name']}")
                        elif name.endswith(('.txt', '.md', '.html')):
                            # Extract lyrics from text files
                            lyrics_entry = {
                                'title': item['name'].replace('.txt', '').replace('.md', '').replace('.html', ''),
                                'content': content,
    ...
    def scan_for_artist_data(self, structure):
        """Scan repository for artist information"""
        logger.info("Scanning for artist data...")
        ...
        for item in structure:
            if item['type'] == 'file':
                ...
                # Look for files that might contain artist info
                if any(keyword in name for keyword in ['artist', 'bio', 'about', 'profile']):
                    logger.info(f"Found potential artist file: {item['name']}")
                    content = self.fetch_file_content(item['path'])
                    ...
                    if content:
                        if name.endswith('.json'):
                            try:
                                data = json.loads(content)
                                artists_data.extend(data if isinstance(data, list) else [data])
                            except json.JSONDecodeError:
                                logger.warning(f"⚠️ Invalid JSON in {item['name']}")
                        else:
                            # Extract artist info from other formats
                            artist_entry = {
                                'name': 'Ilse DeLange',  # Default
                                'bio': content,
                                'source_file': item['path']
                            }
...
"""
        ...
        if len(albums) > 5:
            report += f"- ... and {len(albums) - 5} more\n"
        
        report += f"""
## Lyrics Found
"""
        
        for i, lyric in enumerate(lyrics[:5], 1):  # Show first 5
            title = lyric.get('title', f'Lyric {i}')
            report += f"- {title}\n"
        
        if len(lyrics) > 5:
            report += f"- ... and {len(lyrics) - 5} more\n"
        
        report += f"""
## Images Extracted
"""
        
        for i, image in enumerate(images[:10], 1):  # Show first 10
            name = image.get('name', f'Image {i}')
            size = image.get('size', 0)
            report += f"- {name} ({size} bytes)\n"
        
        if len(images) > 10:
            report += f"- ... and {len(images) - 10} more images\n"
        
        # Calculate total image size
        total_image_size = sum(img.get('size', 0) for img in images)
        report += f"\n**Total Images Size**: {total_image_size:,} bytes ({total_image_size/1024/1024:.2f} MB)\n"
        
        report += f"""
## Next Steps
1. Review extracted content in `migration_data/extracted_content/`
2. Transform HTML content to structured JSON format
3. Convert and optimize images for web use
4. Map old website structure to new website routes
5. Update content in new website
6. Test migration completeness

## File Locations
- **Albums Data**: `migration_data/extracted_content/albums.json`
- **Lyrics Data**: `migration_data/extracted_content/lyrics.json`
- **Images**: `migration_data/extracted_content/images/`
- **Media Files**: `migration_data/extracted_content/media/`
- **Images Inventory**: `migration_data/extracted_content/images_inventory.json`

## Notes
- HTML content needs to be parsed to extract structured data
- Images have been downloaded and saved locally
- Manual review and cleanup may be required
- Some content might need format conversion
