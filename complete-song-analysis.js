...
console.log('🎵 Identifying missing songs from the complete 161 song list...\n');

// The complete list of 161 songs from the provided table
...
console.log(`📊 Complete song list: ${completeSongList.length} songs`);
console.log(`📊 Currently processed: ${processedLyrics.lyrics.length} songs`);

// Create a map of current songs (case-insensitive)
const currentSongMap = new Map();
processedLyrics.lyrics.forEach(song => {
    currentSongMap.set(song.title.toLowerCase().trim(), song);
});

// Find missing songs
const missingSongs = completeSongList.filter(song => {
    const normalizedTitle = song.title.toLowerCase().trim();
    return !currentSongMap.has(normalizedTitle);
});

console.log(`\n❌ Missing songs: ${missingSongs.length}\n`);

// Categorize by suspected artist (based on the original tables provided)
const tclSongs = [
...
missingSongs.forEach(song => {
    if (tclSongs.includes(song.title)) {
        missingSongsByArtist.tcl.push(song);
    } else {
        missingSongsByArtist.ilse.push(song);
    }
});

console.log(`🎤 Missing Ilse DeLange songs: ${missingSongsByArtist.ilse.length}`);
missingSongsByArtist.ilse.forEach((song, index) => {
    console.log(`  ${index + 1}. ${song.title} (${song.filename})`);
});

console.log(`\n🎸 Missing The Common Linnets songs: ${missingSongsByArtist.tcl.length}`);
missingSongsByArtist.tcl.forEach((song, index) => {
    console.log(`  ${index + 1}. ${song.title} (${song.filename})`);
});

// Calculate final counts if we had all songs
const currentIlseCount = processedLyrics.lyrics.filter(s => s.artist === 'Ilse DeLange').length;
const currentTCLCount = processedLyrics.lyrics.filter(s => s.artist === 'The Common Linnets').length;

const projectedIlseCount = currentIlseCount + missingSongsByArtist.ilse.length;
const projectedTCLCount = currentTCLCount + missingSongsByArtist.tcl.length;

console.log(`\n📈 PROJECTED FINAL COUNTS:`);
console.log(`- Ilse DeLange: ${currentIlseCount} → ${projectedIlseCount} (+${missingSongsByArtist.ilse.length})`);
console.log(`- The Common Linnets: ${currentTCLCount} → ${projectedTCLCount} (+${missingSongsByArtist.tcl.length})`);
console.log(`- Total: ${processedLyrics.lyrics.length} → ${projectedIlseCount + projectedTCLCount} (+${missingSongs.length})`);

if (projectedIlseCount === 131 && projectedTCLCount === 30) {
    console.log(`\n✅ PERFECT MATCH! This would give us exactly 131 Ilse + 30 TCL = 161 total`);
} else {
    console.log(`\n⚠️  Projected counts: Ilse ${projectedIlseCount}/131, TCL ${projectedTCLCount}/30`);
}

// Save the missing songs analysis
const analysisData = {
    summary: {
        totalExpected: 161,
        currentExtracted: processedLyrics.lyrics.length,
        missing: missingSongs.length,
        projectedFinal: {
            ilse: projectedIlseCount,
            tcl: projectedTCLCount,
            total: projectedIlseCount + projectedTCLCount
        }
    },
    missingSongs: {
        ilse: missingSongsByArtist.ilse,
        tcl: missingSongsByArtist.tcl,
        total: missingSongs
    },
    completeSongList: completeSongList
};

fs.writeFileSync(
    path.join(__dirname, 'complete-song-analysis.json'),
    JSON.stringify(analysisData, null, 2)
);

console.log(`\n💾 Complete analysis saved to: complete-song-analysis.json`);
console.log(`\n🎯 NEXT STEP: Extract the ${missingSongs.length} missing songs to achieve 100% parity!`);
